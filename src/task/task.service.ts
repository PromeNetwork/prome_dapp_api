...
import { TaskType, Task } from './task.dto';
  ...
  async addTwitterTask(task: Task) {
    try {
      ...
      if (!exist) {
        ...
        if (task.type === TaskType.FOLLOW_TWITTER && task.content) {
          const handle = task.content.split('-')[0];
          await this.pgPool.query(
            `UPDATE account_v2 SET account_tag = $1 WHERE address = $2 `,
            ['@' + handle, task.address.toLowerCase()],
          );
        }
      } else {
        await this.pgPool.query(
    ...
    try {
      ...
      if (completeTasks.length === 5) {
        ...
        if (parent) {
          await this.pgPool.query(
            `INSERT INTO coupon (address, code, serial,status ) VALUES ($1, $2, $3, $4)ON CONFLICT DO NOTHING ;`,
            [parent.parent, `${uuidv4()}`, 2, 0],
          );
          await this.pgPool.query(
            `UPDATE account_v2 SET task_count = task_count + 1 WHERE address = $1;`,
            [parent.parent],
          );
          const lastTask = new Task();
          lastTask.address = parent.parent;
          lastTask.type = TaskType.SHARE;
          lastTask.status = 'complete';
          await this.pgPool.query(
            `INSERT INTO task (address, type, status) VALUES ($1, $2, $3);`,
            [lastTask.address, lastTask.type, lastTask.status],
          );
        }
      }
    } catch (error) {
  ...
  async addTask(task: Task) {
    try {
      if (task.content && typeof task.content === 'object') {
        task.content = JSON.stringify({ content: task.content });
      } else {
        task.content = JSON.stringify({});
      }
